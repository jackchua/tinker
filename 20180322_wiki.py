...
def read_corpus():
    filename = '/home/jack/source/tinker/data/helpmylover/dataset/wiki.train.npy'
...
class TextModel(nn.Module):
    def __init__(self, charcount, args):
        ...
        self.rnns = nn.ModuleList([
            nn.LSTM(input_size=args.embedding_dim, hidden_size=args.embedding_dim, num_layers=3, dropout=0.2, batch_first=True)
        ])
...
#         validation_loss = self.trainer.get_state('training_loss', default=0)
#         validation_error = self.trainer.get_state('training_error', default=0)
...
def train_model(model, train_dataset, valid_dataset, args):
    ...
    train_loader = DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=True, **kw)
    validate_loader = DataLoader(dataset=valid_dataset, batch_size=args.batch_size, **kw)
...
def main(argv):
    ...
    parser.add_argument('--hidden-dim', type=int, default=500, metavar='N',
                        help='Hidden dim')
    parser.add_argument('--embedding-dim', type=int, default=350, metavar='N',
                        help='Embedding dim')
    parser.add_argument('--no-cuda', action='store_true', default=False,
                        help='disables CUDA training')
    args = parser.parse_args(argv)
    ...
    targets = np.load('/home/jack/source/tinker/data/helpmylover/dataset/wiki.train.npy')
    vocabulary = np.load('/home/jack/source/tinker/data/helpmylover/dataset/vocab.npy')
    ...
    train_dataset = TensorDataset(torch.from_numpy(inputs).long(), torch.from_numpy(targets).long())

    # Validation dataset
    targets = np.load('/home/jack/source/tinker/data/helpmylover/dataset/wiki.valid.npy')
    targets = np.concatenate(targets).ravel()
    targets = batchify(targets, args=args)
    inputs = make_inputs(targets)
    valid_dataset = TensorDataset(torch.from_numpy(inputs).long(), torch.from_numpy(targets).long())
    ...
    if not os.path.exists(checkpoint_path):
        ...
        train_model(model=model, train_dataset=train_dataset, valid_dataset=valid_dataset, args=args)
    else:
